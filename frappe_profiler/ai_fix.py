...
"""On-demand LLM-suggested fixes for Profiler Findings.
...
from the ``frappe_profiler.api.suggest_fix`` whitelisted endpoint (request
...
``ai_model`` / ``ai_api_key`` are all overridable in Profiler Settings, so a
...
# from Profiler Settings override these; the "OpenAI-compatible" provider
...
def is_available(section: str | None = None) -> bool:
	...
	Profiler Settings is a hard disable. Fails soft: an unknown ``section`` (or
...
def suggest_fix(finding: dict) -> dict:
	...
	if not provider.get("model"):
		raise AiFixError(
			"No AI model is configured — set 'Model' under Profiler Settings ▸ "
			"AI Fix Suggestions."
		)
	...
	if provider.get("needs_key") and not provider.get("api_key"):
		raise AiFixError(
			...
			"Profiler Settings ▸ AI Fix Suggestions."
		)
...
def humanize_steps(actions: list[dict], *, session_title: str | None = None) -> str:
	...
	if not provider.get("model") or not provider.get("base_url"):
		raise AiFixError(
			...
			"under Profiler Settings ▸ AI Fix Suggestions."
		)
	if provider.get("needs_key") and not provider.get("api_key"):
...
def _resolve_provider() -> dict:
	...
	on every call — it is never cached in ``ProfilerConfig`` and never
	...
	if name not in _PROVIDER_DEFAULTS:
		raise AiFixError(f"Unknown AI provider {name!r}. Pick one in Profiler Settings.")
	...
	try:
		...
		api_key = get_decrypted_password(
			"Profiler Settings", "Profiler Settings", "ai_api_key",
...
def _http_post(url: str, headers: dict, body: dict, *, provider: str, where: str) -> dict:
	...
	if status in (401, 403):
		...
		raise AiFixError("The AI provider rejected the API key — check it in Profiler Settings.")
	if status == 404:
		...
		raise AiFixError(
			...
			f"Profiler Settings is probably missing a path segment.{hint}"
		)
	if status == 429:
		_log_http_error(provider, where, status)
