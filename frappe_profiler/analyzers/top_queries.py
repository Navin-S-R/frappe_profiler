...
DEFAULT_TOP_N = 20
# v0.6.0 Round 6: was a hardcoded constant, now Profiler Settings
...
def _resolve_slow_query_threshold() -> tuple[float, float]:
	...
	Profiler Settings via the cached config; falls back to the legacy
...
def _resolve_tracked_apps() -> tuple[str, ...]:
	"""``Profiler Settings ▸ Tracked Apps`` allowlist, or ``()`` when
