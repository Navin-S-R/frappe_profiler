...
function render_ai_buttons(frm) {
	...
	frappe.call({
		method: "frappe_profiler.api.ai_capabilities",
		callback: (r) => {
			const c = (r && r.message) || {};
...
function subscribe_session_progress(frm) {
	...
	frappe.realtime.on("profiler_progress", (p) => {
		if (!mine(p)) return;
...
// LLM and stores the suggestion on the Profiler Finding row (so it's cached
// and shows up in the regenerated HTML report). Re-opening shows the cached
// suggestion with a "Regenerate" option.
//
// Mirrors AI_ELIGIBLE_FINDING_TYPES in frappe_profiler/ai_fix.py.
...
function _ai_backfill_call(frm, mode) {
	...
	frappe.call({
		method: "frappe_profiler.api.backfill_ai_fixes",
...
function render_humanize_steps_button(frm) {
	...
	frm.add_custom_button(
		...
		() => {
			const run = () =>
				frappe.call({
					method: "frappe_profiler.api.humanize_steps",
...
function open_suggest_index_dialog(frm, tables) {
	...
	const d = new frappe.ui.Dialog({
		...
		primary_action: (values) => {
			d.hide();
			frappe.call({
				method: "frappe_profiler.api.suggest_index",
	...
	const d = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				fieldtype: "HTML",
				options:
					'<p class="text-muted small">This sends the selected finding\'s code snippet, callsite, and SQL to the AI provider configured in <b>Profiler Settings ▸ AI Fix Suggestions</b>. Use a local model (Ollama / LM Studio) there to keep everything on-box.</p>',
			},
			{
				fieldname: "result",
				fieldtype: "HTML",
...
function run_ai_suggest(frm, d, finding_ref, regenerate) {
	...
	frappe.call({
		method: "frappe_profiler.api.suggest_fix",
		args: {
...
function show_phase2_dialog(frm, data) {
	...
	// v0.6.0 Round 6: default reads from Profiler Settings via the
...
function render_download_buttons(frm) {
	...
	// permission gating still applies (Profiler User role + per-File
	// permission hook).
