...
def _reset_profiler_modules():
	for name in list(sys.modules):
		if name == "frappe_profiler" or name.startswith("frappe_profiler."):
...
def fake_frappe(monkeypatch):
	...
	fake.conf = {}
	fake.get_roles = lambda user=None: ["Profiler User"]
	...
	# The tests re-import frappe_profiler.* bound to the FAKE frappe; drop
...
class TestEnqueueRegistersPendingJob:
	...
	def test_does_not_track_the_analyze_job_itself(self, fake_frappe):
		...
		fake_bg.enqueue("frappe_profiler.analyze.run", session_uuid="sess-1")
		assert fake.cache.smembers(_pending_key("sess-1")) == set()
...
class TestBgWaitForPendingJobs:
	def _patch_common(self, monkeypatch, *, pending, wait_seconds, scheduler_disabled=False):
		from frappe_profiler import analyze, session
		...
		monkeypatch.setattr(
			"frappe_profiler.settings.get_config",
	...
	def test_reenqueues_when_jobs_still_running(self, monkeypatch):
		...
		assert len(enq) == 1
		args, kwargs = enq[0]
		assert args[0] == "frappe_profiler.analyze.run"
...
class TestBackgroundJobWaitSeconds:
	def test_default(self):
		...
		assert settings.ProfilerConfig().background_job_wait_seconds == 60
