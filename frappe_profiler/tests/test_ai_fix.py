...
"""Tests for frappe_profiler.ai_fix — the provider-agnostic LLM client
...
class TestResolveProvider:
	def test_anthropic_defaults(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg(ai_provider="Anthropic")):
	...
	def test_kimi_uses_openai_protocol_with_moonshot_default(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg(ai_provider="Kimi (Moonshot)")):
			p = ai_fix._resolve_provider()
		assert p["protocol"] == "openai"
	...
	def test_unknown_provider_raises(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg(ai_provider="Bogus")):
			with pytest.raises(ai_fix.AiFixError):
...
class TestIsAvailable:
	def test_false_when_disabled(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg(ai_enabled=False)):
			assert ai_fix.is_available() is False

	def test_false_when_no_model(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg(ai_enabled=True)), \
		     patch("frappe_profiler.ai_fix._resolve_provider",
...
class TestSuggestFix:
	...
	def test_happy_path_returns_payload(self, monkeypatch):
		...
		with patch("frappe_profiler.ai_fix._resolve_provider", return_value=dict(self._PROVIDER)):
	...
	def test_anthropic_dispatch(self, monkeypatch):
		...
		with patch("frappe_profiler.ai_fix._resolve_provider", return_value=prov):
	...
	def test_missing_model_raises_before_any_http(self, monkeypatch):
		...
		with patch("frappe_profiler.ai_fix._resolve_provider", return_value=bad):
...
def _cfg_ai_on(**overrides):
	"""ProfilerConfig with the master switch + provider config valid; section
	...
	return _settings.ProfilerConfig(**base)


class TestIsAvailableSection:
	def test_all_sections_on_when_config_valid(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on()), \
	...
	def test_findings_section_off_blocks_only_findings(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on(ai_suggest_findings=False)), \
		     patch.object(ai_fix, "_resolve_provider", return_value=_PROVIDER_OK):
			assert ai_fix.is_available() is True
	...
	def test_indexes_section_off_blocks_only_indexes(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on(ai_suggest_indexes=False)), \
		     patch.object(ai_fix, "_resolve_provider", return_value=_PROVIDER_OK):
			assert ai_fix.is_available(section="findings") is True
	...
	def test_humanize_section_off_blocks_only_humanize(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on(ai_humanize_steps=False)), \
		     patch.object(ai_fix, "_resolve_provider", return_value=_PROVIDER_OK):
			assert ai_fix.is_available(section="findings") is True
			assert ai_fix.is_available(section="indexes") is True
	...
	def test_master_switch_off_blocks_everything(self):
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on(ai_enabled=False)), \
		     patch.object(ai_fix, "_resolve_provider", return_value=_PROVIDER_OK):
			assert ai_fix.is_available() is False
	...
	def test_provider_unconfigured_blocks_regardless_of_section(self):
		...
		from frappe_profiler.ai_fix import AiFixError
		with patch("frappe_profiler.settings.get_config", return_value=_cfg_ai_on()), \
