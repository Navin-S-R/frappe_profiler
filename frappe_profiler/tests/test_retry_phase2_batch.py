...
in ``profiler_session.js``. Pattern: source-inspect (mirrors
...
_JS_PATH = os.path.join(
	...
	"profiler_session", "profiler_session.js",
)
...
class TestRetryPhase2BatchJsCall:
	...
	def test_js_calls_batch_endpoint(self):
		js = _read(_JS_PATH)
		assert 'method: "frappe_profiler.api.retry_phase2_analyzes_batch"' in js
