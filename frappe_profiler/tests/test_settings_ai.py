...
"""Tests for the v0.6.0 AI-fix config fields on Profiler Settings.
...
the cached ``ProfilerConfig`` (it's read on demand by ai_fix.py via
...
class TestAiConfigDefaults:
	...
	def test_api_key_is_not_part_of_the_config(self):
		...
		assert not hasattr(settings.ProfilerConfig(), "ai_api_key")
