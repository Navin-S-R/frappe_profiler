...
class TestDisabledThreshold:
	def test_threshold_zero_keeps_everything_in_ms(self):
		...
		with patch(
			...
			return_value=ProfilerConfig(large_duration_threshold_ms=0),
		):
			html = renderer.render_raw(doc, recordings=[])
...
class TestHighThreshold:
	def test_threshold_above_all_values_keeps_everything_in_ms(self):
		...
		with patch(
			"frappe_profiler.settings.get_config",
			return_value=ProfilerConfig(large_duration_threshold_ms=10000),
		):
			html = renderer.render_raw(doc, recordings=[])
...
class TestLowThreshold:
	def test_threshold_500ms_converts_everything_above(self):
		...
		with patch(
			"frappe_profiler.settings.get_config",
			return_value=ProfilerConfig(large_duration_threshold_ms=500),
		):
			html = renderer.render_raw(doc, recordings=[])
