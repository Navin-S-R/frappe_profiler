...
def test_is_frappe_metadata_column_truth_table():
	from frappe_profiler.analyzers.base import is_frappe_metadata_column
...
def test_is_frappe_meta_table_truth_table():
	from frappe_profiler.analyzers.base import is_frappe_meta_table
...
class TestFrappeMetaTablesExcluded:
	...
	def test_render_shows_meta_table_note(self):
		...
		with patch("frappe_profiler.settings.get_config",
		           return_value=ProfilerConfig(hide_framework_tables=False)):
...
class TestHideFrameworkTablesToggle:
	...
	def test_default_on_filters_framework_tables(self):
		from frappe_profiler import renderer
		# Default toggle is True (no patching needed — ProfilerConfig() default).
...
class TestRenderConfigFooter:
	...
	Profiler Settings flag and re-open an existing (un-regenerated) HTML
	...
	def test_footer_stamps_default_on_values(self):
		from frappe_profiler import renderer
		# Default ProfilerConfig() → hide_framework_tables on, no tracked/
