...
"""Tests for the v0.6.0 Round 6 Profiler Settings additions.
...
from frappe_profiler.line_profile import analyzer as lp_analyzer


class TestProfilerConfigDefaults:
...
class TestSlowQueryThresholdResolver:
	...
	def test_default_from_settings(self):
		...
		# When get_config returns an unconfigured ProfilerConfig, the
		# resolver returns the dataclass default (200.0) and a high
		# threshold of 200 * 2.5 = 500.
		fake_cfg = settings.ProfilerConfig()
		with patch("frappe_profiler.settings.get_config", return_value=fake_cfg):
	...
	def test_override_propagates(self):
		...
		fake_cfg = settings.ProfilerConfig(slow_query_threshold_ms=400.0)
		with patch("frappe_profiler.settings.get_config", return_value=fake_cfg):
			slow, high = top_queries._resolve_slow_query_threshold()
		assert slow == 400.0
...
class TestSlowHotPathThresholdResolver:
	...
	def test_legacy_defaults_when_settings_unreachable(self):
		...
		with patch("frappe_profiler.settings.get_config", side_effect=RuntimeError):
			med_pct, med_ms, high_pct, high_ms = call_tree._resolve_hot_path_thresholds()
...
class TestRendererMinActionFilter:
	...
	def test_zero_threshold_keeps_all_actions(self):
		...
		fake_cfg = settings.ProfilerConfig(min_action_duration_ms=0.0)
		with patch("frappe_profiler.settings.get_config", return_value=fake_cfg):
	...
	def test_threshold_drops_short_actions(self):
		...
		fake_cfg = settings.ProfilerConfig(min_action_duration_ms=10.0)
		with patch("frappe_profiler.settings.get_config", return_value=fake_cfg):
			html = renderer.render_raw(doc, recordings=[])
