...
# path containing "frappe/" or "frappe_profiler/", so the "user code" frame
...
class TestRenderTimeCallsiteResolution:
	def test_repeated_hot_frame_resolves_to_file_line_and_snippet(self):
		...
		doc = _fake_doc([_repeated_hot_frame("frappe_profiler/renderer.py::render")])
		html = renderer.render_raw(doc, recordings=[])
		assert "frappe_profiler/renderer.py:" in html      # resolved callsite line
	...
	def test_function_not_invoked_shows_def_line(self):
		doc = _fake_doc([_function_not_invoked("frappe_profiler.renderer.render")])
		html = renderer.render_raw(doc, recordings=[])
		assert "frappe_profiler/renderer.py:" in html
