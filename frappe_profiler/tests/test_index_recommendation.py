...
# sys.modules['frappe'] / drops frappe_profiler.* can't trigger a *re-import*
...
class TestRecommendedIndex:
	def _analyze(self, recordings):
		from frappe_profiler.analyzers import table_breakdown as tb
	...
	def test_caps_composite_width(self):
		from frappe_profiler.analyzers.table_breakdown import MAX_RECOMMENDED_INDEX_COLS
...
def test_is_write_hot_table_truth_table():
	from frappe_profiler.analyzers.base import is_write_hot_table
...
class TestRenderedIndexCandidatePanel:
	...
	def test_no_ai_index_block_when_indexes_section_toggle_off(self):
		...
		with patch("frappe_profiler.settings.get_config",
		           return_value=settings.ProfilerConfig(ai_suggest_indexes=False)):
			html = renderer.render_raw(_doc([entry]), recordings=[])
...
class TestAiSuggestIndex:
	...
	def test_suggest_index_happy_path(self, monkeypatch):
		...
		with patch("frappe_profiler.ai_fix._resolve_provider", return_value=dict(_PROVIDER)):
...
class TestAnalyzeIndexHelpers:
	...
	def test_enrich_table_breakdown_adds_ai_index_when_enabled(self, monkeypatch):
		...
		monkeypatch.setattr("frappe_profiler.settings.get_config",
