...
"bake AI fixes into the report" step (Profiler Settings ▸ AI Fix
...
def _patches(cfg, *, available=True, suggest=None):
	...
	return (
		patch("frappe_profiler.settings.get_config", return_value=cfg),
		patch("frappe_profiler.ai_fix.is_available", return_value=available),
		patch("frappe_profiler.ai_fix.suggest_fix", side_effect=suggest),
	)
...
class _Row(SimpleNamespace):
	"""A stand-in for a Profiler Finding child row."""
...
class TestBackfillAiSuggestions:
	...
	def test_backfills_eligible_rows_without_a_suggestion(self):
		...
		assert fk.db.writes[0][0] == "Profiler Finding" and fk.db.writes[0][2] == "llm_fix_json"
