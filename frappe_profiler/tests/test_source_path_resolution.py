...
class TestResolveSourcePath:
	def test_resolves_app_relative_path(self):
		...
		# frappe.get_app_path; under pytest (cwd = apps/frappe_profiler) the
		# cwd-relative short form already exists and is returned as-is. Either
		# way it must point at the real file.
		resolved = renderer._resolve_source_path(_APP_REL)
		assert resolved is not None and os.path.exists(resolved)
		assert os.path.abspath(resolved).endswith(
			os.path.join("apps", "frappe_profiler", "frappe_profiler", "renderer.py")
		)
	...
	def test_none_for_nonexistent_app_relative(self):
		assert renderer._resolve_source_path("frappe_profiler/this_does_not_exist_xyzq.py") is None
...
class TestAiPayloadForFinding:
	def test_gets_source_window_and_phase2_hotline_for_hot_path_finding(self):
		...
		phase2_index = {("renderer.py", "render"): {
			...
			"run_uuid": "r", "dotted_path": "frappe_profiler.renderer.render",
		}}
