...
context: when a Profiler Finding row carries an ``llm_fix_json``, the card
...
never takes the lazy file-read path (which imports ``frappe_profiler.analyze``
...
class TestAiFixBlockRendering:
	...
	def test_no_block_when_findings_section_toggle_off(self):
		...
		with patch("frappe_profiler.settings.get_config",
		           return_value=settings.ProfilerConfig(ai_suggest_findings=False)):
			html = renderer.render_raw(doc, recordings=[])
		assert "Suggested fix (AI" not in html
		assert "review before applying" not in html
		# Sanity: with the toggle back on, the block IS there.
		with patch("frappe_profiler.settings.get_config",
		           return_value=settings.ProfilerConfig(ai_suggest_findings=True)):
