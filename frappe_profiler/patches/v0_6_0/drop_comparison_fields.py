...
Removes two fields from Profiler Session:
  - compared_to_session
  - is_baseline

Frappe's auto-DDL drops the columns once profiler_session.json no longer
...
def execute():
	...
	# field removal from profiler_session.json. After this, the underlying
	# tabProfiler Session table no longer has compared_to_session /
	# is_baseline columns.
	try:
		frappe.reload_doc("frappe_profiler", "doctype", "profiler_session")
	except Exception:
		frappe.log_error(title="v0.6.0 patch: reload profiler_session (drop comparison)")
	...
	try:
		titles = frappe.get_all("Profiler Session", pluck="title") or []
