...
Removes three fields:
  - Profiler Session.safe_report_file
  - Profiler Session.safe_report_pdf_file
...
def execute():
	...
	# field removal from profiler_session.json / profiler_settings.json.
	# After this call, the underlying tabProfiler Session table no longer
	# has safe_report_file / safe_report_pdf_file columns, and
	# tabProfiler Settings no longer has safe_report_include_source_lines.
	try:
		frappe.reload_doc("frappe_profiler", "doctype", "profiler_session")
	except Exception:
		frappe.log_error(title="v0.6.0 patch: reload profiler_session")

	try:
		frappe.reload_doc("frappe_profiler", "doctype", "profiler_settings")
	except Exception:
		frappe.log_error(title="v0.6.0 patch: reload profiler_settings")
