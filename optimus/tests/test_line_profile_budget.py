...
def test_disengage_releases_tool_and_sets_flag(env):
	if HAS_MON:
...
def test_started_watchdog_disengages_after_budget(env):
	# A real Timer fires the disengage, which frees a registered tool 2.
	sys.monitoring.use_tool_id(PID, "line_profiler")
	sys.monitoring.set_events(PID, sys.monitoring.events.LINE)
	cap.start_overhead_watchdog("r3", 0.05)
	time.sleep(0.25)
	assert sys.monitoring.get_tool(PID) is None  # flow freed mid-request
